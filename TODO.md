...
- [x] `--template` / `--template-vars`: send from a full `.eml` file or an HTML body template with Go `text/template` variable substitution
  - Shared loading/expansion in the new `internal/common/template` package: `ParseVars` (`key=value`), `Render` (`text/template` with `missingkey=error`), `IsEML` (extension check, case-insensitive), and `ParseEML` (`net/mail` + MIME walk extracting From/To/Cc/Bcc/Subject/Message-ID, first text/plain and text/html bodies with base64/quoted-printable decoding, and the list of unmapped headers). Each protocol's `RunE` resolves the template via a per-protocol `resolveTemplate` after validation.
  - `--template <file>` on all five sendmail protocols (SMTP, gmail, msgraph, EWS, JMAP; env `*TEMPLATE`): `.eml` extension → EML mode, anything else → HTML body mode. Mutually exclusive with `--body`, `--bodyhtml`, and (gmail/msgraph) `--body-template`; `--template-vars key=value` (repeatable, env `*TEMPLATEVARS`, `{{.key}}` in the template) requires `--template`.
  - **EML mode, raw injection** — SMTP: rendered message injected verbatim into `DATA` (envelope from `--from`/`--to`/`--cc`/`--bcc`, falling back to the EML headers; verified end-to-end against a local sink incl. dot-stuffing). Gmail: rendered EML base64url-encoded verbatim into `Message.Raw` (deviates from the original parse-and-map plan — raw is higher fidelity since the Gmail API natively accepts RFC 822). For both, flags whose content the file already carries (attachments/headers/priority, and for gmail recipients/subject) are rejected in validation.
  - **EML mode, parse-and-map** — msgraph/EWS/JMAP: recognised fields (recipients where flags weren't given, subject, text/HTML bodies, JMAP From) are mapped onto the native send call; the From header is ignored where the protocol dictates the sender (Graph `--mailbox`, EWS authenticated user), and Bcc (EWS) plus unmapped headers are logged as skipped.
  - **HTML body mode**: rendered content becomes `BodyHTML` on all five protocols, extending the Gmail/Msgraph-only `--body-template` (which remains, without variable substitution).
  - Tests: `internal/common/template/template_test.go` (vars, render, EML parsing incl. multipart/encodings) and `internal/protocols/smtp/sendmail_template_test.go` (validation exclusions, HTML/EML resolution, flag-vs-EML precedence). Docs: `docs/protocols/{smtp,gmail,msgraph,ews,jmap}.md`, `docs/config-file.md`.
