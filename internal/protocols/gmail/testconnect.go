...
// gmailConnectEndpoint is a lightweight Gmail API path used only to prove
...
// a discovery endpoint (large body).
const gmailConnectEndpoint = "https://gmail.googleapis.com/gmail/v1/users/me/profile"

// gmailConnectTimeout bounds the whole probe (dial + TLS handshake + response).
const gmailConnectTimeout = 30 * time.Second

// testConnect performs an unauthenticated HTTP/TLS probe against the Gmail API
// endpoint. No credentials or mailbox are required — any HTTP status (401
// expected) confirms reachability.
//
// For a global service like gmail.googleapis.com this test cannot verify auth or
...
func testConnect(ctx context.Context, config *Config, csvLogger logger.Logger, slogLogger *slog.Logger) error {
	...
	client := &http.Client{Transport: transport, Timeout: gmailConnectTimeout}

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, gmailConnectEndpoint, nil)
	if err != nil {
		writeConnectCSV(csvLogger, slogLogger, config, "", 0, "", "", nil, 0, err.Error())
		return err
	}

	logDebug(slogLogger, "Probing Gmail endpoint", "url", gmailConnectEndpoint, "proxy", config.ProxyURL)
...
func writeConnectCSV(
	...
	if logErr := csvLogger.WriteRow([]string{
		config.Action, status, gmailConnectEndpoint,
